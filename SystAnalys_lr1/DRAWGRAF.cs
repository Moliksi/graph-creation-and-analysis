...
namespace GRAF
{
    class DrawGraph
    {
        ...
        public void drawEdge(Vertex V1, Vertex V2, Edge E, int numberE)
        {
            if (E.v1 == E.v2)
            {
                gr.DrawArc(E.color, (V1.x - 2 * R), (V1.y - 2 * R), 2 * R, 2 * R, 90, 270);
            ...
            {
                gr.DrawLine(E.color, V1.x, V1.y, V2.x, V2.y);
        ...
        public void drawALLGraph(List<Vertex> V, List<Edge> E)
        {
            ...
            for (int i = 0; i < E.Count; i++)
            {
                if (E[i].v1 == E[i].v2)
                {
                    gr.DrawArc(E[i].color, (V[E[i].v1].x - 2 * R), (V[E[i].v1].y - 2 * R), 2 * R, 2 * R, 90, 270);
                    point = new PointF(V[E[i].v1].x - (int)(2.75 * R), V[E[i].v1].y - (int)(2.75 * R));
                    gr.DrawString(((char)('a' + i)).ToString(), fo, br, point);
                }
                else
                {
                    gr.DrawLine(E[i].color, V[E[i].v1].x, V[E[i].v1].y, V[E[i].v2].x, V[E[i].v2].y);
